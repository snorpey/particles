var ParticleSystem = function()
{
	...
	function update()
	{
		...
		{		
			...
			if(
				_particles[i].getPosition().x > 0 &&
				_particles[i].getPosition().x < _size.width &&
				_particles[i].getPosition().y > 0 &&
				_particles[i].getPosition().y < _size.height
			)
			{
				_particles[i].applyRepulsionForce(Vector.create([0, _particles[i].getPosition().y, 0]), _particle_radius, 0.65);
				_particles[i].applyRepulsionForce(Vector.create([_particles[i].getPosition().x, 0, 0]), _particle_radius, 0.65);
				_particles[i].applyRepulsionForce(Vector.create([_size.width, _particles[i].getPosition().y, 0]), _particle_radius, 0.65);
				_particles[i].applyRepulsionForce(Vector.create([_particles[i].getPosition().x, _size.height, 0]), _particle_radius, 0.65);
			}
			
			if(_particles[i].getPosition().x > _size.width)
			{
				_particles[i].setPosition(_size.width - _particle_radius + 3, _particles[i].getPosition().y, 0);
			}
			
			if(_particles[i].getPosition().y > _size.height)
			{
				_particles[i].setPosition(_particles[i].getPosition().x + 3, _size.height - _particle_radius, 0);
			}
			
			if(_particles[i].getPosition().x < 0)
			{
				_particles[i].setPosition(_particle_radius + 3, _particles[i].getPosition().y, 0);
			}
			
			if(_particles[i].getPosition().y < 0)
			{
				_particles[i].setPosition(_particles[i].getPosition().x, _particle_radius + 3, 0);
			}
